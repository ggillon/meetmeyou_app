...
Future<Contact> getContact(User currentUser, {required String cid}) async {
  return await FirestoreDB(uid: currentUser.uid)
      .getContact(currentUser.uid, cid);
}

// Delete a particular contact
Future<void> deleteContact(User currentUser, {required String cid}) async {
  return await FirestoreDB(uid: currentUser.uid)
      .deleteContact(currentUser.uid, cid);
}
...
Future<List<String>> getContactsCIDs(
  User currentUser,
) async {
  List<String> CIDs = [];
  final contactList =
      await FirestoreDB(uid: currentUser.uid).getContacts(currentUser.uid);
...
Future<Contact> updatePrivateContact(
  User currentUser,
  String cid, {
  String? displayName,
  String? firstName,
...
Future<Contact> createNewGroupContact(User currentUser,
    {required String displayName, String? photoURL, String? about}) async {
  Contact contact = Contact(
    cid: cidGenerator(),
    uid: currentUser.uid,
    displayName: displayName,
    ...
    phoneNumber: '',
    photoURL: photoURL ??
        'https://i2.wp.com/9to5google.com/wp-content/uploads/sites/4/2021/07/new-google-groups-logo.png?ssl=1',
...
Future<Contact> updateGroupContact(User currentUser, String cid,
    {String? displayName, String? photoURL, String? about}) async {
  Contact contact = await updatePrivateContact(currentUser, cid,
      displayName: displayName, photoUrl: photoURL, about: about);
  await FirestoreDB(uid: currentUser.uid).setContact(currentUser.uid, contact);
  return contact;
}

// Group contact functions : add one or more contacts, returns the group as contact
Future<Contact> addToGroup(User currentUser, String cid,
    {String? contactCid, List<String>? contactListCids}) async {
...
Future<Contact> removeFromGroup(User currentUser, String cid,
    {String? contactCid, List<String>? contactListCids}) async {
  Contact group = await getContact(currentUser, cid: cid);
  if (contactCid != null && group.group != null) {
...
Future<void> inviteProfile(User currentUser, {required String uid}) async {
  ...
  Contact invited =
      contactFromProfile((await db.getProfile(uid))!, uid: currentUser.uid);
  ...
  Contact invitation =
      contactFromProfile(await getUserProfile(currentUser), uid: uid);
  invitation.status = CONTACT_INVITATION; // change status to invitation contact
  await db.setContact(uid, invitation);
}

Future<void> respondProfile(User currentUser,
    {required String cid, required bool accept}) async {
  Database db = FirestoreDB(uid: currentUser.uid);
  if (accept) {
    // Accepted invitation
...
Future<void> linkProfiles(
  ...
  Contact contact1 =
      contactFromProfile((await db.getProfile(uid))!, uid: currentUser.uid);
  Contact contact2 =
      contactFromProfile((await db.getProfile(currentUser.uid))!, uid: uid);
